...
[![Tests](https://img.shields.io/badge/tests-587-green)](DEVELOPMENT.md#8-测试)
[![API Cost](https://img.shields.io/badge/API-0%C2%A0per%C2%A0hour-brightgreen)](ARCHITECTURE.md#10-成本模型)

> **核心理念**：调用一次，固化一次，永久免费。LLM 只改"激励规则"，不写"行动方案"。

E-Agent 是一个 Minecraft Fabric 服务端 AI 模组。LLM 只在三种时机被调用（启动预置/10 分钟心跳/卡死诊断），其他时间 Bot 全凭本地反射 + 激素 + 感知做决策，零 API 成本。
...
# 发送自然语言指令（LLM 理解 → 蒸馏为反射 → 永久 0 成本执行）
/ai Steve 帮我挖 10 个铁矿
...
```
1. /ai spawn Steve        → 先生成一个熟手（含 L0 生存反射）
2. /ai Steve 砍木头       → LLM 理解 → 固化 dig_oak_log 反射 → 下次砍树 0 成本
3. 观察 Steve 自动挖矿    → 已固化的反射自动触发，不调 LLM
4. /ai reflex export      → 导出经验包，跨 Bot 复用
5. /ai Steve 死了怎么办？ → LLM 分析死亡原因 → 固化预警反射 → 下次避开
```
...
|------|------|
| `/ai spawn <name>` | 生成假人 |
| `/ai <name> <text>` | 对假人发指令（可对话、可指挥、可提问） |
| `/ai status` | 查看假人实时状态 |
| `/ai challenge start/stop/status` | 15 天生存挑战赛 |
| `/ai reflex export/import` | 导出/导入反射包 |
| `/ai playstyle load/export <name>` | 玩法包切换 |
| `/ai setkey <key>` | 设置 LLM API 密钥 |
...
MemoryManager 管"是什么"（0 成本查询），ConditionedReflex 管"怎么做"（0 成本执行）。MemoryGraph 管记忆间关系（因果/时序/相似/对比），零侵入 MemoryEntry 字段，边缘推理仅在 salience ≥ 0.6 时触发，成功反射后 Hebbian 强化强化关联边权重，支持骨骼导出/导入跨 bot 迁移记忆骨架。三者分离 = LLM 不需要理解上下文。
...
| `CLARIFICATION` | `ambiguity_detected / possible_interpretations / missing_info / suggested_question` | TemplateMatcher |
...
| **激素层** | 秒~分钟 | `HormonalSystem` (NE/DA/5-HT/ACh 4维向量 + 旧别名向下兼容) |
| **反射层** | 分钟~小时 | `ConditionedReflex` + 双权重(stw/ltb) 固化 |

执行反射 → 成功/失败 → 激素浓度变化 → 视角选择偏移 → 反射固化 → 死亡 → 三规则继承给后代。

**图式压缩 (Schema Compression)**：WorkingMemoryPool 记录每 tick 坐标 + 满意度 (1200 buffer)。缓冲区满或心跳触发时，4×4 网格聚类提取 3 个热度最高区域 → Schema 对象 → MemoryGraph 持久化。LLM 读取后输出 LandscapePatch 间接影响 ActionSorter，不直驱。
...
|------|------|
| **cortex/** | 规划、复杂决策、语义理解 |
| **hippocampus/** | 记忆存储、高光回忆、记忆关系图 |
| **amygdala/** | 条件反射、学习、评价 |
| **brainstem/** | 先天反射、基础动作、生存本能 |

**骨架** (`bayesian/`, `commend/`, `config/`, `util/`): 跨领域支撑层。`util/TagResolver` 提供实体别名→ID 解析 (`entity_aliases.json`) + 方块/物品/实体分类 + Minecraft Tag 查询。

### 降级执行层 (DegradedExecutor) — 永不卡死

当所有反射均不匹配时，硬编码安全兜底自动触发：healthUrgency → flee/eat → seekShelter → collectFood → digWood。冷却 100 tick，0 API。

### 执行后反思 (ReflectionEngine)

每次行动成败自动记录为结构化 ReflectionRecord，永久存入 `reflections/<reflexId>.json`。`PatternSummary` 追踪 totalAttempts/successRate/consecutiveFailures。连续 5 次失败或成功率 <30% 自动降权 shortTermWeight。

### 类别模糊匹配

`TagResolver.sharesCategory()` 让 `dig_oak_log` 反射自动对 `birch_log` 生效。回退链：精确匹配 → 类别匹配 → 余弦相似度 → 图扩散。
...
记忆间自动推理因果/时序/相似/对比四种关系边，salience ≥ 0.6 时触发。支持 BFS 遍历、因果链追踪、时间线排序、贝叶斯重排序。零侵入 MemoryEntry 字段，独立文件 `memory_graph.json` 持久化。

扩展功能：

| 功能 | Phase | 说明 |
|------|:-----:|------|
| Hebbian 强化 | 1 | 反射成功后加强关联记忆节点边的权重 (`reinforcePath(nodeIds, delta)`) |
| 扩散激活 | 2 | 候选耗尽时通过 BFS 低权重剪枝探索关联记忆 (`traverse` w/ minWeight) |
| 骨骼导出/导入 | 3 | 轻量摘要跨 bot 迁移 (`exportSkeleton` / `importSkeleton` 集成 ReflexPackManager) |
| **图式压缩** | **1** | **栖息地轨迹 (1200 tick buffer) → 网格聚类 → 抽象图式 (Schema) → MemoryGraph 持久化。离线重放压缩连续轨迹为离散区域，LLM 读取后生成 LandscapePatch，跨窗口领地意识。** |
...
```
minecraft/eagent/
├── config/                全局配置 JSON (10 个文件: knowledge_base, entity_aliases, intent_map, template_patterns, category_display, stop_words, chat_templates, idle_words, memory_triggers, challenge_items)
├── thresholds/            自适应阈值
...
└── bots/{bot_uuid}/
    ...
    ├── alarms/            L1 一次预警
    ├── memory/            高光记忆
    ├── memory_graph.json  记忆关系图 (因果/时序/相似/对比)
...
- [HARDWARE_REQUIREMENTS.md](HARDWARE_REQUIREMENTS.md) — 硬件与成本要求
